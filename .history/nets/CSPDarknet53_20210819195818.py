...
LastEditTime: 2021-08-19 19:58:18
LastEditors: TuZhou
Description: 
FilePath: \my_yolov5\nets\CSPDarknet53.py
'''
...
class CSP2(nn.Module): 
    def __init__(self, in_channels, out_channels, number_block, groups=1, gw=0.5):  
        ...
        self.conv2 = nn.Sequential(*[self.convBlock.append([CBL(c_, c_, kernel_size = 1), CBL(c_, c_, kernel_size = 3)])\
                                        for _ in range(number_block - 1)])
    ...
    def forward(self, x):
        ...
        #作为短接边
        y2 = self.conv4(x)
        return self.conv5(self.activation(self.bn(torch.cat((y1, y2), dim=1))))
...
class CSPDarknet(nn.Module):
    ...
    def forward(self, x):
        ...
        #out3 = self.spp(self.conv4(out2))
        out3 = self.csp2(self.spp(self.conv4(out2)))

        #输出的特征层尺寸由大到小排列，out1为76x76
...
#   如读取预训练文件等
#------------------------------#
def preprocess_backbone(pretrained, yamls_path='./nets/yolov5s.yaml', **kwargs):
    number_blocks, filters_info = process_yaml(yamls_path)
