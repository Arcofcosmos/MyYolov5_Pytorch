...
LastEditTime: 2021-08-18 21:47:42
LastEditors: TuZhou
Description: 
FilePath: \my_yolov5\nets\CSPDarknet53.py
'''
...
class CSPDarknet(nn.Module):
    def __init__(self, image_channel, number_blocks, filters_info):
        ...
        self.block1 = CSP1(filters_info[1][0], filters_info[2][0], number_blocks = number_blocks[0])
        
        self.conv2 = CBL(filters_info[2][0], filters_info[3][0], kernel_size = filters_info[3][1], stride = filters_info[3][2])

        self.block2 = CSP1(filters_info[3][0], filters_info[4][0], number_blocks = number_blocks[1])

        self.conv3 = CBL(filters_info[4][0], filters_info[5][0], kernel_size = filters_info[5][1], stride = filters_info[5][2])

        self.block2 = CSP1(filters_info[5][0], filters_info[6][0], number_blocks = number_blocks[2])

        self.conv3 = CBL(filters_info[6][0], filters_info[7][0], kernel_size = filters_info[7][1], stride = filters_info[7][2])

        self.spp = SPP(filters_info[7][0], filters_info[8][0], filters_info[8][1])

        self.csp2 = CSP2(filters_info[8][0], filters_info[9][0], number_blocks = number_blocks[3])
...
#   读取出backbone模块的信息
#------------------------------#
def process_yaml(yaml_path = './yolov5s.yaml'):
