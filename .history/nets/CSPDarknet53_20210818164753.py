...
LastEditTime: 2021-08-18 16:47:53
LastEditors: TuZhou
Description: 
FilePath: \my_yolov5\nets\CSPDarknet53.py
'''
...
# CSP Bottleneck https://github.com/WongKinYiu/CrossStagePartialNetworks
    def __init__(self, in_channels, out_channels, number_block, groups=1, gw=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.resblock = nn.Sequential(*[Resblock(c_, c_,  e=1.0) for _ in range(n)])

    def forward(self, x):
        y1 = self.cv3(self.resblock(self.cv1(x)))          #经过残差层的直连边
        y2 = self.cv2(x)            #经过一次卷积的短接边
