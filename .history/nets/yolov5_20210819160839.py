...
LastEditTime: 2021-08-19 16:08:39
LastEditors: TuZhou
Description: 
FilePath: \my_yolov5\nets\yolov5.py
'''
...
class YoloBody(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', image_channels=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        self.block1 = CSP2(self.filters_info[0][1]*2, self.filters_info[2][0], self.number_blocks[0])
    ...
    def forward(self, x):
        ...
        #经过head部分装换结果输出通道维度
        out0 = nn.conv2(self.filters_info[5][0], self.result_channels, 1, 1)
        out1 = nn.conv2(self.filters_info[7][0], self.result_channels, 1, 1)
        out2 = nn.conv2(self.filters_info[9][0], self.result_channels, 1, 1)
