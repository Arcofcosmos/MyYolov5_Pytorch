...
LastEditTime: 2021-08-21 11:01:02
LastEditors: TuZhou
Description: 
...
def fit_one_epoch(net, yolo_loss,epoch,train_iteration,val_iteration,train_dataLoder,genval,Epoch,cuda):
    ...
    torch.save(model.state_dict(), 'trained_model/Epoch%d-Total_Loss%.4f-Val_Loss%.4f.pth'%((epoch+1),total_loss/(train_iteration+1),val_loss/(val_iteration+1)))
...
if __name__ == "__main__":
    ...
    model_cfg_path = yaml_dict['model_cfg_path']
    ...
    if anchors_flag:
        ...
        print('My anchors have been created!\n')
    ...
    num_classes = yaml_dict['number_classes']
    ...
    #------------------------------------------------------#
    model = YoloBody(yaml_path = model_cfg_path, number_anchors = len(anchors[0]), gw = gw)
    weights_init(model)

    #------------------------------------------------------#
    #   yolov5未使用预训练权重，从coco数据集从头训练
    #------------------------------------------------------#
    model_path = "trained_model/yolov5s.pt"
    print('Loading weights into state dict...')
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model_dict = model.state_dict()
    pretrained_dict = torch.load_state_dict(model_path, map_location=device)
    ...
    loss_history = LossHistory("trained_model/")

    #----------------------------------------------------#
    #   获得图片路径和标签
    #----------------------------------------------------#
    annotation_path = yaml_dict['annotation_path']
    #----------------------------------------------------------------------#
    ...
    freeze_flag = yaml_dict['freeze_flag']          #表示是否使用冻结训练
    ...
    #------------------------------------------------------#
    if True:
        lr              = yaml_dict['freeze_lr']
        Batch_size      = yaml_dict['freeze_batch']
        Init_Epoch      = 0
        Freeze_Epoch    = yaml_dict['freeze_epoch']
    ...
    if True:
        # if freeze_flag:
        #     lr              = 0.0001
        #     Batch_size      = yaml_dict['unfreeze_batch']
        #     Freeze_Epoch    = yaml_dict['freeze_epoch']
        #     Unfreeze_Epoch  = yaml_dict['unfreeze_epoch']
        #else:
        lr              = yaml_dict['unfreeze_lr']
        Batch_size      = yaml_dict['unfreeze_batch']
        Freeze_Epoch    = yaml_dict['freeze_epoch']
        Unfreeze_Epoch  = yaml_dict['unfreeze_epoch']
