...
Author: TuZhou
Version: 1.0
Date: 2021-08-19 18:53:52
LastEditTime: 2021-08-20 18:34:08
LastEditors: TuZhou
Description: 
...
def box_iou(b1, b2, x1y1x2y2=False, GIoU=False, DIoU=False, CIoU=False, eps=1e-7):
    """
    输入为：
    ----------
    ...
    """
    if not x1y1x2y2:
        #将x,y,w,h装换为x1,y1,x2,y2即左上角右下角的形式
        # 求出预测框左上角右下角
        b1_xy = b1[..., :2]
        b1_wh = b1[..., 2:4] + eps
        b1_wh_half = b1_wh/2.
    ...
    intersect_area = intersect_wh[..., 0] * intersect_wh[..., 1]           #计算交集
    b1_area = b1_wh[..., 0] * b1_wh[..., 1]
    b2_area = b2_wh[..., 0] * b2_wh[..., 1]
    union_area = b1_area + b2_area - intersect_area  + eps                      #计算并集
    iou = intersect_area / union_area               #算出iou


    if GIoU or DIoU or CIoU:
        #计算出包含b1,b2的最小框c的宽高
        cw = torch.max(b1_maxes[..., 0], b2_maxes[...,0]) - torch.min(b1_mins[...,0], b2_mins[...,0])
        ch = torch.max(b1_maxes[...,1], b2_maxes[...,1]) - torch.min(b1_mins[...,1], b2_mins[...,1])
        if CIoU or DIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            if CIoU:
                ...
                ciou = ciou - alpha * v
                return ciou
            else:       #计算DIoU: https://arxiv.org/abs/1911.08287v1
                c1 = cw ** 2 + ch ** 2 + eps
                r1 = ((b2_maxes[0] + b2_mins[0] - b1_maxes[0] - b1_mins[0]) ** 2 +
                            (b2_maxes[1] + b2_mins[1] - b1_maxes[1] - b1_mins[1]) ** 2) / 4
                return iou - r1 / c1
                
        else:                #计算GIOU：https://arxiv.org/pdf/1902.09630.pdf
            c_area = cw * ch + eps
...
class YOLOLoss(nn.Module):
    ...
    def forward(self, input, targets=None):
        ...
        #   计算预测结果和真实结果的GIOU
        #----------------------------------------------------------------#
        ciou = (1 - box_iou( pred_boxes_for_ciou[mask.bool()], t_box[mask.bool()], GIoU = True))* box_loss_scale[mask.bool()]
